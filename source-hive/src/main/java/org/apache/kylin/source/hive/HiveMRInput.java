...
public class HiveMRInput implements IMRInput {
    ...
    //如何读取该表数据,以及返回每一行列组成的字符串数组
    @Override
    public IMRTableInputFormat getTableInputFormat(TableDesc table) {
        return new HiveTableInputFormat(table.getIdentity());//table.getIdentity()返回database.table格式
    }

    /**
     * 构造函数构建一个databases.table,因此该类提供了mr读取一个table的输入源,每一行记录调用parseMapperInput方法,返回列组成的集合数组
     */
    public static class HiveTableInputFormat implements IMRTableInputFormat {
         ...
         * @param fullQualifiedTableName "databaseName.tableName" 用数据库.表名字组成的参数,可能没有database名字
         * 返回数据库和table名字组成的数组
         */
        ...
        public void configureJob(Job job) {
            try {
                ...
                job.setMapOutputValueClass(org.apache.hive.hcatalog.data.DefaultHCatRecord.class);//map的输出对象
            } catch (IOException e) {
